...
public:
	...
	Data(int dia, int mes, int any) : m_dia(dia), m_mes(mes), m_any(any) {m_dia = dia; m_mes = mes; m_any = any;}
	...
	int getAny() { return m_any; }
	void llegeix();
	bool dataValida() const;

	Data operator+(int numero);
	bool operator<(Data datilla);
